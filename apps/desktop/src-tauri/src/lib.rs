// The Core sidecar plumbing only exists in a RELEASE bundle; in dev the Core
// runs as a separate process (`pnpm core:dev`). Gating these imports release-only
// keeps debug builds warning-free.
