...
import type { AgentSummary, OrgMember, OrgRole } from "../../lib/api";
 ...
 * Extends the wire `org.status` shape with a `lastSyncedAt` field populated
 * separately from `org.synced` events (the server does not include it in
 * org.status directly).
 */
...
export function OrgSection({
  ...
  function handleToggleMembers(): void {
    if (!membersExpanded) onRequestMembers();
  ...
  return (
    <>
      ...
      <div className="org-actions-row">
        ...
        {confirmLeave ? (
          <div className="org-leave-confirm">
            <span>Remove all org agents and leave? This cannot be undone.</span>
      ...
      >
        ...
        {orgMembers.length > 0 && ` (${orgMembers.length})`}
      </button>
      {membersExpanded && orgMembers.length > 0 && (
